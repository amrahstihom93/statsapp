...
def makeProcess(request):
	...
	try:
		if request.method == 'POST':
			...
			if request.POST['parent_process'] == 'root':
				form.parent_p_id = 'PID'+request.POST['parent_process']+'00000000000000'
			else:
...
def getProcessList(request):
	...
	try:
		...
		responseObj = []
		usr = request.user
		if request.POST['parent_process'] =='PIDroot00000000000000':
			rootID =  Process.objects.get(user_id=usr.id,parent_p_id='PID00000000000000')
			print('root from process1',rootID.process_id)
			list = Process.objects.filter(user_id=usr.id,parent_p_id=rootID.process_id).values()
		else :
